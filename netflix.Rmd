...
A continuación, creamos nuevas variables temporales a partir de la fecha de valoración `Date`. Para cada valoración obtenemos el año, el mes, el día, la semana y el día de la semana.
...
Creamos una tabla con la cantidad, la suma, la media, la mediana, la moda y la desviación típica de las valoraciones de cada película.
...
```{r formattable, echo = T, max.height='100px'}
films_table %>%
...
ggplot(data) +
  ...
  xlab("Día de la semana") +
  ylab("Proporción") +
  ggtitle("Distribución de los valores 
          de las puntuaciones por día de la semana") +
...
Primero se genera una tabla con las 250 película y sus puntuaciones distribuidas desde 1999 hasta 2005.
...
users %>%
  ggplot() +
  geom_point(mapping = aes(x = count, y = mean_score))

# OK
users %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = count, y = mean_score, group = cut_interval(count, 5)))
# COMENTARIO: A mayor número de valoraciones, la nota media es menor. Puede ser debido a que sean más críticos o que vean películas de serie B.

# miramos outliers, solo hay 5 usuarios con más de 150 valoraciones
users %>%
  filter(count > 150)
```
...
#### Análisis peliculas antiguas
* Las pelis viejas se valoran más que las nuevas?
```{r old_movie_1, message = FALSE}

old_movies <- data %>%
  group_by(Score, Release_Year) %>%
  summarise(count = n())

old_movies %>%
  ggplot(aes(x = Release_Year, y = Score)) + 
  geom_point(aes(size = count))
#Se puntuan más las peliculas a partir del ~1985
```
...
old_movies_v2 %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = Release_Year, y = mean_score, group = cut_interval(Release_Year, 5)))
#Las películas más antiguas antes de 1960 tienen mejor score, pero también las ha visto menos gente.
```


#### Otros datos interesantes a incluir en un futuro dataset
**Tengo que escribir esto mejor**
A nivel de los usuarios sería interesante incluir datos como el sexo, la edad, el país de los usuarios y otros datos socioeconómicos con el fin de poder diferenciar mejor sus preferencias.

A nivel de las películas también seria interesante ver el número total de visualizaciones que no han sido puntuadas, la productora cinematográfica a la cual pertenecen, la duración de la película...
