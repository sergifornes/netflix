...
data %>%
  ...
  ggplot() +
  geom_boxplot(mapping = aes(x = Release_Year, y = count, group = cut(Release_Year, breaks = seq(1925, 2005, 5)))) +
  xlab("Año de estreno") +
...
data %>%
  ...
  summarise(mean_score = mean(Score)) %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = Release_Year, y = mean_score, group = cut(Release_Year, breaks = seq(1925, 2005, 10)))) +
  xlab("Año de estreno") +
