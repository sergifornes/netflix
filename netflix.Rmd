...
names(df) <- c("User","Score","Date","ID_film")
```
...
nombres_pelis <- nombres_pelis %>% separate(X1,",",into =c("ID_film","Year","Title"), extra="merge", convert = TRUE)

nombres_pelis$Year <- ifelse(nombres_pelis$Year == "NULL", NA,nombres_pelis$Year)
nombres_pelis <- nombres_pelis %>% mutate(Year = as.integer(Year))

# FALTA JUNTAR NOMBRES_PELIS Y DF Y DEJAR EL DATAFRAME BONITO

df_final <- inner_join(nombres_pelis, df)


write_csv(df_final, "../data/pelis.csv")
