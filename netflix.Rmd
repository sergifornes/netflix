...
users <- users %>%
  mutate(p95 = ifelse(users$count <= quantile(users$count, 0.95), "p95", "!p95"),
         p99 = ifelse(users$count <= quantile(users$count, 0.99), "p99", "!p99"))
...
ggplot(users) +
  geom_freqpoly(mapping = aes(x = mean_score, y = ..density.., color = p95))

ggplot(users) +
  geom_freqpoly(mapping = aes(x = mean_score, y = ..density.., color = p99))

# ? o estandarizar?
data %>%
  mutate(p95 = ifelse(User %in% p95$User, "p95", "!p95")) %>%
  group_by(Score, p95) %>%
  ggplot() +
  geom_bar(mapping = aes(x = Score, fill = p95), position = "dodge")
```
...
### Cosas a hacer ###
Distancia? (Clusters entre individuos)
¿Qué otros datos serían interesantes? Ej. Sexo, Edad, País...
¿Otros análisis con los datos que ya tenemos?
